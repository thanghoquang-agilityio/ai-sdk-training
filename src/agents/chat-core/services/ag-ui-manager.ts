...
function tryParseDirectMutationPrompt(text: string): Omit<PendingToolCall, "specialist"> | null {
  const match = text.trim().match(
    /^(Approve|Reject)\s+(.+?)\.\s*(?:Reason|Comment):\s*(.+?)\.?\s*$/i,
  );
  ...
  return {
    name,
    args: { requestQuery, comment, showTeamPending: true },
