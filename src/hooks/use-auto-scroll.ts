...
export function useChatAutoScroll(
  ...
  useEffect(() => {
    ...
    // Skip when a user message was just appended and we're not loading.
    const lastMessage = messages.at(-1);
    if (!isLoading && lastMessage?.role === "user") return;

    const isStartingToLoad = isLoading && lastMessage?.role === "user";
    ...
    scheduledRef.current = requestAnimationFrame(() => {
      ...
      c.scrollTo({ 
        top: c.scrollHeight, 
        behavior: isStartingToLoad ? "instant" : "smooth" 
      });
    });
